...
**Optional rear-view video** (independent pipeline, never blocks the side view): `POST /api/runs/{id}/rear-video` → `rear_worker.process_rear_video` (own Celery queue `rear_view`) → `rear_job_runner` → `rear_metrics` (landmarks low-pass filtered first; knee alignment (`knee_valgus` key) peak in early stance, step width as % of hip width, left/right symmetry over those two; hip drop (measured from each leg's own initial contact) and pronation are computed but kept under `legs.<leg>.experimental` and never shown — on real clips the hip landmarks tilt opposite to a pelvic drop, so hip drop is unvalidated; `meta.metrics_version` 3 = this shape, 2 = hip drop still top-level, absent = the original; the frontend hides hip drop/pronation for every version and rebuilds the balance score without them (`frontend/lib/rearBalance.ts`); all as % of gait cycle, not synchronised with the side video) + `rear_confidence` (wider bands, pattern-not-angle framing, no error margins until we measure our own). Results go to `run_videos.results_json` and are merged into `results.rear_view` at read time (`results_schema.py`, schema in `backend/schema/results.schema.json`). Regenerate the sample with `python scripts/generate_rear_sample.py`.
