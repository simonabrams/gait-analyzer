"""
Generate a matplotlib multi-panel dashboard from the results dict. Returns a
Figure; caller saves to PNG. In a web app, could return figure or export to
bytes for streaming.
"""
...
def create_dashboard(results):
    """
    Build a multi-panel figure from results (summary, strides, flags).

    Args:
        results: Full results dict (from metrics + heuristics, or from results.json).

    Returns:
        matplotlib.figure.Figure (caller saves with fig.savefig(...)).
    """
    ...
    # Knee angle at each foot strike (bar chart)
    ...
    # Summary panel: averages and pass/fail
